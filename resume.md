...
- 喻智勇/男/1998
- 学历: 大专 (西安交通大学计算机科学与技术专业)
- 博客: https://i-want-tobelieve.github.io/
- github: https://github.I-Want-ToBelieve.io/
...
自学前端并积累了整套前端自学资源, 会基础的后端开发。九年编码经验, 五年开发经验，涉猎广泛，熟悉前端框架选型，脚手架搭建，自动化测试，持续集成持续部署，[gitlab][] 私服搭建, 可以配置各种脚手架提高组员开发效率和开发体验,日常使用 [linux][] 发行版声明式系统 [nixos][] 进行开发, 并业余从事 [nixos][] 开源社区的建设, 熟悉函数式编程理念，写得一手同事都夸赞的干净代码。
...
- 工作内容: 负责公司设备维护，完成教培项目的开发维护与任务

- 在职时间: 2023.11 - 2024.08
...
我需要对我管理的五十多台笔记本进行统一运维，应此采用了[nixos-anywhere][] 在五十多台笔记本上一键部署了 [nixos][] 系统 ，为完成以下几个项目提供系统环境：

1. 投屏系统：该项目需要实现将一台主机的屏幕投屏到数十台主机上，并且需要支持超高清和低延迟，前端采用 [react][] 和 [typescript][] 的技术栈编写, 通过 `navigator.mediaDevices.getDisplayMedia` API 获取屏幕画面 ，后端则是 [node][] 与 koa， 它们通过 websocket 协议通信。

2. 群控系统：该项目需要实现一台电脑远程控制其他主机，并在多台主机之间切换，这里采用了支持 [wayland][]显示协议的 sunshine 和 moonlight 的开源方案

3. 锁屏系统：该项目需要实现对多台主机分组，并按分组锁定屏幕与解锁，这里桌面应用采用  [electron][] & [react][] & [typescript][] 的技术栈，后台程序采用 [node][] 与 koa, 它们通过 socket.io 封装的 websocket 协议通信。

4. 浏览器自动化系统：该项目需要实现在多台分组后的主机上自动打开浏览器执行自动化上号等爬虫任务。

5. 题库考试系统：该项目需要实现题目的分类录入还有一键通过题库生成在线考试。
